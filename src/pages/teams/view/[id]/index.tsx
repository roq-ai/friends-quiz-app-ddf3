...
import { deleteTeamQuizById } from 'apiSdk/team-quizs';
...
function TeamViewPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<TeamInterface>(
    ...
    () =>
      getTeamById(id, {
        relations: ['user', 'team_member', 'team_quiz'],
      }),
  );
  ...
  const team_quizHandleDelete = async (id: string) => {
    setDeleteError(null);
    try {
              ...
              {hasAccess('team_quiz', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    ...
                    <NextLink passHref href={`/team-quizs/create?team_id=${data?.id}`}>
                      <Button colorScheme="blue" mr="4" as="a">
                        Create
                      </Button>
                    </NextLink>
                  ...
                  <TableContainer mb={4}>
                    <Table variant="simple">
                      ...
                      <Tbody>
                        {data?.team_quiz?.map((record) => (
                          <Tr
                            cursor="pointer"
                            onClick={() => router.push(`/team-quizs/view/${record.id}`)}
                            key={record.id}
                          >
                            <Td>{record.score}</Td>
                            <Td>
                              {hasAccess('team_quiz', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                                <NextLink
                                  onClick={(e) => e.stopPropagation()}
                                  passHref
                                  href={`/team-quizs/edit/${record.id}`}
                                >
                                  <Button mr={2} as="a" variant="outline" colorScheme="blue" leftIcon={<FiEdit2 />}>
                                    Edit
                                  </Button>
                              ...
                              {hasAccess('team_quiz', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                                <IconButton
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    team_quizHandleDelete(record.id);
                                  }}
                                  colorScheme="red"
                                  variant="outline"
                                  aria-label="edit"
